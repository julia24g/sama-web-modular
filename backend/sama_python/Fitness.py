...
def fitness(X, input_object):
    Eload = input_object.Eload
    Ppv_r = input_object.Ppv_r
    Pwt_r = input_object.Pwt_r
    Cbt_r = input_object.Cbt_r
    Cdg_r = input_object.Cdg_r
    T = input_object.T
    Tc_noct = input_object.Tc_noct
    G = input_object.G
    c2 = input_object.c2
    fpv = input_object.fpv
    Gref = input_object.Gref
    Tcof = input_object.Tcof
    Tref = input_object.Tref
    Ta_noct = input_object.Ta_noct
    G_noct = input_object.G_noct
    n_PV =  input_object.n_PV
    gama =  input_object.gama
    Vw = input_object.Vw
    h_hub = input_object.h_hub
    h0 = input_object.h0
    alfa_wind_turbine = input_object.alfa_wind_turbine
    v_cut_in = input_object.v_cut_in
    v_cut_out = input_object.v_cut_out
    v_rated = input_object.v_rated
    R_B = input_object.R_B
    Q_lifetime = input_object.Q_lifetime
    ef_bat = input_object.ef_bat
    b = input_object.b
    C_fuel = input_object.C_fuel
    R_DG = input_object.R_DG
    TL_DG = input_object.TL_DG
    MO_DG = input_object.MO_DG
    SOC_max = input_object.SOC_max
    SOC_min = input_object.SOC_min
    SOC_initial = input_object.SOC_initial
    n_I = input_object.n_I
    DC_AC_ratio=input_object.DC_AC_ratio
    Grid = input_object.Grid
    Cbuy = input_object.Cbuy
    a = input_object.a
    LR_DG = input_object.LR_DG
    Pbuy_max = input_object.Pbuy_max
    Psell_max = input_object.Psell_max
    self_discharge_rate = input_object.self_discharge_rate
    alfa_battery = input_object.alfa_battery
    c = input_object.c
    k = input_object.k
    Imax = input_object.Imax
    Vnom = input_object.Vnom
    RE_incentives = input_object.RE_incentives
    C_PV = input_object.C_PV
    C_WT = input_object.C_WT
    C_DG = input_object.C_DG
    C_B = input_object.C_B
    C_I=input_object.C_I
    C_CH=input_object.C_CH
    Engineering_Costs=input_object.Engineering_Costs
    n=input_object.n
    L_PV=input_object.L_PV
    R_PV=input_object.R_PV
    ir=input_object.ir
    L_WT=input_object.L_WT
    R_WT=input_object.R_WT
    L_B=input_object.L_B
    L_I=input_object.L_I
    R_I=input_object.R_I
    L_CH=input_object.L_CH
    R_CH=input_object.R_CH
    MO_PV=input_object.MO_PV
    MO_WT=input_object.MO_WT
    MO_B=input_object.MO_B
    MO_I=input_object.MO_I
    MO_CH=input_object.MO_CH
    RT_PV=input_object.RT_PV
    RT_WT=input_object.RT_WT
    RT_B=input_object.RT_B
    RT_I=input_object.RT_I
    RT_CH=input_object.RT_CH
    CO2=input_object.CO2
    NOx=input_object.NOx
    SO2=input_object.SO2
    E_CO2=input_object.E_CO2
    E_SO2=input_object.E_SO2
    E_NOx=input_object.E_NOx
    Annual_expenses=input_object.Annual_expenses
    Service_charge=input_object.Service_charge
    Csell=input_object.Csell
    Grid_Tax=input_object.Grid_Tax
    System_Tax=input_object.System_Tax
    EM=input_object.EM
    LPSP_max=input_object.LPSP_max
    RE_min=input_object.RE_min
    Budget=input_object.Budget
    Grid_escalation = input_object.Grid_escalation
    C_fuel_adj = input_object.C_fuel_adj
    Grid_Tax_amount = input_object.Grid_Tax_amount
    Grid_credit = input_object.Grid_credit
    NEM = input_object.NEM
    NEM_fee = input_object.NEM_fee
    if X.size == 1:
        X = X[0]
