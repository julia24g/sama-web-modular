...
def battery_model(Nbat, Eb,  alfa_battery,c, k, Imax, Vnom, ef_bat):
    ...
    Pch_max3 = np.full(Eb.shape, Nbat * Imax * Vnom / 1000)

    Pdch_max = (k*Q1*exp(-k*dt) + Q*k*c*(1-exp(-k*dt))) / (1-exp(-k*dt) + c*(k*dt-1+exp(-k*dt))) * np.sqrt(ef_bat)

    Pch_max = np.fmin(np.fmin(Pch_max1, Pch_max2), Pch_max3) / np.sqrt(ef_bat)
